...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        # Pie chart for all launch sites – show total successful launches per site
        fig = px.pie(
            spacex_df,
            names='Launch Site',
            values='class',
        ...
        outcome_counts['class'] = outcome_counts['class'].replace({0: 'Failed', 1: 'Success'})
        # Create pie chart
        fig = px.pie(
            outcome_counts,
            names='class',
...
def update_scatter_plot(selected_site, payload_range):
    # Filter based on payload slider range
